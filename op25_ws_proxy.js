...
function initOp25WsProxy() {
  if (!config.OP25_API_WS_URL) {
    log.debug("OP25_API_WS_URL not set; OP25 WebSocket proxy disabled.");
    return null;
  }
  log.debug(`Enabling OP25 WS proxy to ${config.OP25_API_WS_URL}`);
  op25Wss = new WebSocket.Server({ noServer: true });

  op25Wss.on("connection", (ws) => {
    log.debug("OP25 WebSocket client connected");

    const op25Client = new WebSocket(config.OP25_API_WS_URL);

    op25Client.on("open", () => {
      log.debug("Connected to OP25 WS endpoint");
    ...
    ws.on("close", () => {
      op25Client.close();
      log.debug("Client WebSocket closed; closing OP25 WS connection.");
    });

    op25Client.on("close", () => {
      ws.close();
      log.debug("OP25 WS connection closed; closing client socket.");
    });
...
function registerUpgradeHandler(server) {
  if (!config.OP25_API_WS_URL) {
    log.debug(
      "OP25_API_WS_URL not set; skipping upgrade handling for OP25 WS."
    );
    return;
  }
  server.on("upgrade", (request, socket, head) => {
    if (request.url.startsWith("/op25ws")) {
      op25Wss.handleUpgrade(request, socket, head, (ws) => {
        log.debug("Upgraded connection on /op25ws");
